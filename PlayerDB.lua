...
local function EnsurePlayer(name)
    ...
    elseif not addon.PlayerDB[name].name then
        addon.PlayerDB[name].name = name
    end
end

-- Ensure all entries have their name field set
function addon:EnsureNameFields()
    for n, data in pairs(self.PlayerDB) do
...
--- Populate PlayerDB with members of the current group/raid
function addon:PopulatePlayerDBFromGroup()
    local changed = false
    ...
    local function addUnit(unit)
        ...
        if name then
            ...
            if not self.PlayerDB[name] then
                EnsurePlayer(name)
                changed = true
            end
            self.PlayerDB[name].class = class
        end
    end
...
function addon:SetPlayerField(name, field, value)
    ...
    EnsurePlayer(name)
    self.PlayerDB[name][field] = value
...
function addon:GetPlayerDB(name)
    EnsurePlayer(name)
    return self.PlayerDB[name]
end

-- Broadcast the complete PlayerDB table to the raid.
function addon:BroadcastPlayerDB()
    if self.playerDB and self.playerDB.global then
        self.playerDB.global.playerData = self.PlayerDB
    end
    if not self.isMasterLooter then return end
    -- Send to everyone in the current group/raid
    self:SendCommand("group", "playerData", self.PlayerDB)
end
...
local function InitializePlayerDB(playerName, class)
    if not PlayerDB[playerName] then
        ...
        for k in pairs(keepFields) do
            if player[k] == nil then
                InitializePlayerDB(playerName, class)
                return
            end
        end
    end
