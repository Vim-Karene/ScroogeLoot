...
function SLVotingFrame:GetFrame()
	...
	if addon.isMasterLooter then
		b1:SetScript("OnClick", function() if active then LibDialog:Spawn("SCROOGELOOT_CONFIRM_ABORT") else self:Hide() end end)
	else
...
do
	...
	function SLVotingFrame.RightClickMenu(menu, level)
		...
		if level == 1 then
			...
			info.text = L["Award"]
			info.func = function()
				LibDialog:Spawn("SCROOGELOOT_CONFIRM_AWARD", {
					session,
				  	candidateName,
					data.response,
					nil,
