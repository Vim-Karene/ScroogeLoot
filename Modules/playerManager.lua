...
local function CreatePlayerEditRow(parent, playerKey, data)
    local group = AceGUI:Create("InlineGroup")
    group:SetTitle(playerKey)
    ...
    parent.editors[playerKey] = editors
end
...
local function BuildOptions(parent)
    ...
    parent.editors = {}
    for name, data in pairs(addon.PlayerData) do
        CreatePlayerEditRow(parent, name, data)
    end
    ...
    saveBtn:SetCallback("OnClick", function()
        for playerKey, edits in pairs(parent.editors) do
            if addon.PlayerData[playerKey] then
                local pd = addon.PlayerData[playerKey]
                local newName = edits.name:GetText()
                if newName ~= playerKey then
                    addon.PlayerData[playerKey] = nil
                    addon.PlayerData[newName] = pd
                    pd = addon.PlayerData[newName]
                end
                pd.name = newName
                pd.class = edits.class:GetText()
                pd.SP = tonumber(edits.SP:GetText()) or 0
                pd.DP = tonumber(edits.DP:GetText()) or 0
                pd.attended = tonumber(edits.attended:GetText()) or 0
                pd.absent = tonumber(edits.absent:GetText()) or 0
                pd.item1 = edits.item1:GetText()
                pd.item1received = edits.item1received:GetValue()
                pd.item2 = edits.item2:GetText()
                pd.item2received = edits.item2received:GetValue()
                pd.item3 = edits.item3:GetText()
                pd.item3received = edits.item3received:GetValue()

                local a, b = pd.attended, pd.absent
                pd.attendance = (a + b > 0) and math.floor((a / (a + b)) * 100) or 0
            end
        end
        addon:BroadcastPlayerData()
        addon:Print(L["Player Management"]..": "..L["Save"].."!")
    end)
    parent:AddChild(saveBtn)
